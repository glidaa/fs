        ...
        if (!app.isLoading) {
          if (user.state === AuthState.SignedIn) {
            if (username && projectPermalink) {
              ...
              const reqUserProjects = allProjects.filter(x => x.owner === username)
              if (reqUserProjects.length > 0) {
                const reqProject = reqUserProjects.filter(x => x.permalink === projectPermalink)[0]
                if (reqProject) {
                  dispatch(appActions.handleSetProject(reqProject.id, false))
                    if (notePermalink) {
                      dispatch(notesActions.handleFetchNotes(reqProject.id)).then(notes => {
            ...
            if (projectPermalink) {
              const reqProject = Object.values(projects.owned)
                .filter(x => x.permalink === projectPermalink)[0]
              if (reqProject) {
                dispatch(appActions.handleSetProject(reqProject.id, false))
              } else {
                history.replace("/")

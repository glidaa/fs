...
export const setProjects = projects => {
  window.localStorage.setItem("cachedData", JSON.stringify({ ...getCache(), projects }))
}

export const setTasksByProjectID = (projectID, tasks) => {
  window.localStorage.setItem("cachedData", JSON.stringify({ ...getCache(), tasks: { ...getCache().tasks, [projectID]: tasks } }))
}

export const setCommentsByTaskID = (taskID, comments) => {
  window.localStorage.setItem("cachedData", JSON.stringify({ ...getCache(), comments: { ...getCache().comments, [taskID]: comments } }))
}

export const setUser = (user) => {
  window.localStorage.setItem("cachedData", JSON.stringify({ ...getCache(), user }))
}

export const setUsers = (users) => {
  window.localStorage.setItem("cachedData", JSON.stringify({ ...getCache(), users }))
}